(() => {
  ...
  const RELEASE_MARKER = 'GC_V571_CONFIRM';
    ...
    document.querySelectorAll('.recent-address-control').forEach(control => {
      ...
      control.addEventListener('click', event => {
        ...
        if (event.target.closest('.recent-clear')) {
          const message = COMMON['最近地址清除確認'] || '確定要清除全部最近使用地址嗎？';
          if (window.confirm(message)) clearRecentAddresses();
        }
      });
    });
    refreshRecentAddressControls();
    ...
    app.innerHTML = `
      ...
      <section class="form-card">
        <div class="form-head">
          ...
          <div class="test-version-badge">V5.7.1 測試版｜送出前會再次確認</div>
        </div>
