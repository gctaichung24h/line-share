(() => {
  ...
  const RELEASE_MARKER = 'GC_V5715_RECENT_TOUCH_FIRST_TAP_FIX';
  ...
  function bindRecentAddressControls() {
    document.querySelectorAll('.recent-address-control').forEach(control => {
      ...
      const togglePanel = (preservedTop = null) => {
      ...
      toggle?.addEventListener('touchend', event => {
        ...
        const anchorTop = toggleAnchorTop;
        toggleAnchorTop = null;
        blurActiveEditor();
        setTimeout(() => togglePanel(anchorTop), 80);
      }, { passive: false });
